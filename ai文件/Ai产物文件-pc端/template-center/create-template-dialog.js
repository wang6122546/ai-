(function () {
  ...
  function renderCreateTemplateDialog(mode = createDialogMode) {
    ...
    state.creation = { templateType: DEFAULT_TEMPLATE_TYPE, workCategoryId: '', workTypeId: '', workLevelIds: [], name: '', code: '', category: '', icon: '♨', applyStandardConfig: true, status: '草稿', version: 'V1' };
    document.querySelector('.modal-card').className = 'modal-card create-template-v2';
    document.querySelector('#modalBody').innerHTML = `<div class="dialog-heading"><div><h2>创建作业模板</h2><p>选择作业类别与类型，创建草稿后进入设计。</p></div></div><div class="create-form"><div class="template-type-readonly"><span>模板类型　<b>${TEMPLATE_TYPE_LABELS[DEFAULT_TEMPLATE_TYPE]}</b></span><code>${DEFAULT_TEMPLATE_TYPE}</code></div>${mode !== 'normal' ? stateBlock(mode) : !hasGroups ? stateBlock('unavailable') : `<label><span><i>*</i> 作业类别</span><select id="createCategory"><option value="">请选择作业类别</option>${groups.map(g=>`<option value="${g.id}">${g.name}</option>`).join('')}</select></label><label><span><i>*</i> 作业类型</span><select id="createWorkType" disabled><option value="">请先选择作业类别</option></select><small class="field-help" id="typeAvailability">已创建模板的作业类型仍会展示，但不可重复选择。</small></label><label><span>作业等级</span><select id="createLevel" disabled><option value="">请先选择作业类型</option></select></label><label><span>模板名称</span><input id="createName" type="text" readonly placeholder="选择作业类型后自动生成"></label><label><span>模板编码</span><input id="createCode" type="text" readonly placeholder="创建时由系统自动生成"></label><label class="standard-config-choice"><input id="applyStandardConfig" type="checkbox" checked><span><b>应用当前作业类型的标准配置</b><small>加载已发布的风险、措施、安全交底、结束确认标准和默认流程。</small></span></label>`}</div><div class="dialog-footer"><button class="ghost" id="cancelCreate">取消</button>${mode==='error'?'<button class="secondary" id="retryCreateConfig">重试</button>':''}<button class="primary" id="confirmCreate" ${mode!=='normal'||!hasGroups?'disabled':''}>创建并进入设计</button></div>`;
    ...
    const category = document.querySelector('#createCategory'), type = document.querySelector('#createWorkType'), level = document.querySelector('#createLevel'), name = document.querySelector('#createName'), code = document.querySelector('#createCode'), note = document.querySelector('#typeAvailability'), confirm = document.querySelector('#confirmCreate');
    category.onchange = () => { state.creation.workCategoryId = category.value; state.creation.workTypeId = ''; state.creation.workLevelIds = []; state.creation.name = ''; state.creation.code = ''; name.value = ''; code.value = ''; level.innerHTML = '<option value="">请先选择作业类型</option>'; level.disabled = true; const group = groups.find(g=>g.id===category.value); if (!group) { type.innerHTML='<option value="">请先选择作业类别</option>'; type.disabled=true; return; } const available=group.types.filter(x=>!templateExists(x)); type.innerHTML=`<option value="">请选择作业类型</option>${group.types.map(x=>`<option value="${x.id}" ${templateExists(x)?'disabled':''}>${x.name}${templateExists(x)?'（已创建模板）':''}</option>`).join('')}`; type.disabled=!group.types.length; note.textContent=available.length?'已创建模板的作业类型已置灰，不可重复选择。':'该类别下暂无可创建模板的作业类型'; note.classList.toggle('warn',!available.length); state.creation.category=group.name; };
    type.onchange = () => { const group=groups.find(g=>g.id===category.value), selected=group?.types.find(x=>x.id===type.value); state.creation.workTypeId=selected?.id||''; state.creation.name=selected?.name||''; state.creation.code=selected?uniqueCode(selected):''; name.value=state.creation.name; code.value=state.creation.code; const levels=(selected?.levels||[]).map(x=>typeof x==='string'?x:x.name).filter(Boolean); level.innerHTML=levels.length?levels.map(x=>`<option value="${x}">${x}</option>`).join(''):'<option value="未分级">未分级</option>'; level.disabled=!selected; state.creation.workLevelIds=selected?(levels.length?[levels[0]]:[]):[]; };
    level.onchange=()=>{state.creation.workLevelIds=level.value==='全部等级'?['ALL']:(level.value==='未分级'?[]:[level.value]);};
